...
//                 variant="standard"
//                 value={username}
//                 helperText="This is your Channel Name and it cannot be changed"
...
import { createTheme, ThemeProvider } from '@mui/material/styles';
import {
  Stack, TextField, styled, Box, Typography, Paper, Button, Card, CardMedia,
  InputLabel, GlobalStyles, CircularProgress, Snackbar, Alert as MuiAlert, Grid
} from '@mui/material';
import { PhotoCamera } from '@mui/icons-material';
import useMediaQuery from '@mui/material/useMediaQuery';
import { useRecoilState, useSetRecoilState } from 'recoil';
...
const ImageUpload = ({ label, image, onChange }) => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: isSmallScreen ? 'column' : 'row', alignItems: 'center', marginTop: 2 }}>
      <CustomInputLabel sx={{ mr: 2, minWidth: 100 }}>{label}</CustomInputLabel>
      <input
        accept="image/*"
        style={{ display: 'none' }}
      ...
      {image && (
        isSmallScreen ? (
          <Typography sx={{ mt: 2, color: theme.palette.text.primary }}>{image.name}</Typography>
        ) : (
          <Card sx={{ maxWidth: 100, ml: 2, mt: 2 }}>
            <CardMedia component="img" height="100" image={URL.createObjectURL(image)} alt={`${label} Preview`} />
          </Card>
        )
      )}
    ...
    <>
      <ThemeProvider theme={theme}>
        ...
        <Box sx={{
          ...
          alignItems: 'center',
          justifyContent: 'center',
          padding: isSmallScreen ? 2 : 0 // Adjust padding for small screens
        }}>
          <Paper sx={{
            width: isSmallScreen ? "90%" : "40%", // Adjust width based on screen size
            padding: isSmallScreen ? 2 : 4, // Adjust padding based on screen size
            boxShadow: '0 8px 16px rgba(0, 0, 0, 0.3)',
            borderRadius: 3,
            background: '#ffffff'
          }}>
            <Stack spacing={2} margin='auto' width='100%'>
              <Typography variant={isSmallScreen ? "h5" : "h4"} align="center" color="primary">Signup Form</Typography>
              <CustomTextField
                label="Full Name"
                variant="standard"
                value={fullname}
